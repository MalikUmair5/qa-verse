...
function ProjectDetailPage({ projectId, projectData, onBack }: ProjectDetailPageProps) {
  ...
  const [isLoading, setIsLoading] = useState(!projectData) // Only show loading if no projectData is provided
  ...
  // Simulate loading data only if no projectData is provided
  useEffect(() => {
    if (!projectData) {
  ...
  // Use projectData if provided, otherwise show error
    ...
    return (
      <div className='min-h-screen bg-[#FFFCFB] flex items-center justify-center'>
        <div className='text-center'>
          ...
          <p className='text-[#171717] mb-6'>The project you're looking for could not be loaded.</p>
          <motion.button
            onClick={handleBack}
            className='px-6 py-3 bg-[#A33C13] text-white rounded-lg font-medium hover:bg-[#8a2f0f] transition-colors'
  ...
  // Map the actual project data to display format
  const project = projectData ? {
    id: projectData.id.toString(),
    title: projectData.title,
    description: projectData.description,
    category: projectData.category,
    difficulty: 'Medium' as const, // This could be derived from category or added to API
    bugsReported: 0, // This would come from bug reports API
    activeTesters: 0, // This would come from testers API
    ...
    ], // Default instructions - this could be added to API
    testingFocusAreas: [projectData.category], // Using category as focus area
    techStack: projectData.technology_stack.split(',').map(tech => tech.trim()), // Split comma-separated string
