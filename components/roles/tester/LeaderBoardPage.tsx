...
const TopTesterCard: React.FC<TopTesterProps> = ({ name, rank, xp, avatar_url, bgColor }) => {
  const getCardHeight = () => {
  ...
  return (
    ...
    >
      ...
      <div className={`w-16 h-16 md:w-24 md:h-24 rounded-full bg-white md:mb-4 flex items-center justify-center overflow-hidden shadow-md flex-shrink-0
        ...
        {avatar_url ? (
          <Image src={avatar_url} alt={name} width={96} height={96} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full bg-[#A33C13] flex items-center justify-center text-white text-2xl md:text-4xl font-bold uppercase">
            {name.charAt(0)}
          </div>
        )}
      </div>
...
const TesterRow: React.FC<TesterRowProps> = ({ rank, name, username, xp, avatar_url }) => {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center space-x-2 md:space-x-4 flex-1 min-w-0">
        ...
        {/* Profile Image with Fallback */}
        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gray-300 overflow-hidden flex-shrink-0 flex items-center justify-center">
          {avatar_url ? (
            <Image src={avatar_url} alt={name} width={40} height={40} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full bg-[#A33C13] flex items-center justify-center text-white text-xs md:text-sm font-bold uppercase">
              {name.charAt(0)}
            </div>
          )}
        </div>
