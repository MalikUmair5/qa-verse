...
import { verifyPasswordResetOTP } from "@/lib/api/auth/passwordReset";
...
export default function VerifyOTPForm({ 
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          showToast.error('Reset code expired. Please request a new one.');
          setTimeout(() => onBackToRequest(), 2000);
          return 0;
        }
        return prev - 1;
      });
  ...
  const onSubmit = async (data: OTPFormData) => {
    setLoading(true);
    const loadingToast = showToast.loading('Verifying code...');
